...
log("highlight memory verses?" + lss("hi_mm_d", "get"), "color:red", "normal");
...
ch_set.addEventListener("change", function(e) {
  ...
  var dream = dreams[e.target.selectedIndex + 1].split(/[0-9]+/);
  for (var j = 0; j < dream.length; j++) {
    for (var i = 0; i < chlist[season].books.length; i++) {
      if (
        e.target.selectedOptions[0].label.split(" ")[0] ===
        chlist[season].books[i].bk
      ) {
        append(
          e.target.selectedOptions[0].label.split(" ")[0] +" "+
            e.target.selectedOptions[0].label.split(" ")[1] +
            ":" +
            (j + 1) +
            " " +
            dream[j],
...
hi_u_d.addEventListener("input", function(event) {
  ...
  ch = ch_set.value;
  dreamsList.innerHTML = "";
  console.log(ch_set.options.selectedIndex);
  var dream = dreams[ch_set.options.selectedIndex + 1].split(/[0-9]+/);
...
var append = function append(dream, id) {
  ...
  console.log(
    ...
    chlist[season].used1,
    chlist[season].used2,
    chlist[season].smemlist,
    chlist[season].mmemlist
  );
  if (JSON.parse(hi_u)) {
  ...
  if (JSON.parse(hi_mm)) {
    for (var i = 0; i < chlist[season].mmemlist.length; i++) {
      if (
        id.toLowerCase() === chlist[season].mmemlist[i].r1.toLowerCase() ||
        id.toLowerCase() === chlist[season].mmemlist[i].r2.toLowerCase()
      ) {
